...
La idea de \textit{Bagging} \cite{Breiman1996} es calcular $\hat{f}^1(x),\hat{f}^2(x),...,\hat{f}^B(x)$ usando B muestras \textit{bootstrap}\footnote{El bootstrap es un método de remuestreo que se utiliza para cuantificar la incertidumbre o reducir la varianza de un estimador. Consiste en generar múltiples muestras simuladas a partir de un único conjunto de datos mediante extracciones aleatorias con reemplazo. Al utilizar el reemplazo, cada nueva muestra mantiene el tamaño del conjunto original, pero altera la distribución al repetir ciertas observaciones y excluir otras.} y promediar las predicciones para reducir la varianza. Es decir
\begin{equation}
...
%#TODO: Descomposición del error? -> listo! quedó más explicíto
...
%#TODO: b=n? -> Listo! creo que ahora esta mas claro. Validar si N=k=n es decir que cada muestra tiene que tener la misma cantidad de observaciones que la original, repitiendo algunas
...
%#TODO: El gradiente? - Listo! Creo que quedo mas claro
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \begin{equation}
        -g_{ib} = -\left[\frac{\partial L(y_i, f(x_i))}{\partial f(x_i)}\right]_{f=\hat{f}_{b-1}}
        \end{equation}
        \item Ajustar el modelo base $h_b(x,\theta_b)$ a los valores $-g_{ib}$. % <--- ¡Aquí está la corrección del signo!
