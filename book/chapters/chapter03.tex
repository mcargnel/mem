Como se mencionó anteriormente, dos de los problemas principales de los árboles de decisión son la baja capacidad predictiva y la inestabilidad. Por lo que en este capítulo se estudian métodos para evitar estos problemas, en particular \textit{boosting} y \textit{bagging} con dos implementaciones clásicas como \textit{Random Forest} y \textit{Gradient Boosting Machines}. Estas técnicas mejoran la capacidad predictiva y reducen la variabilidad al agrupar múltiples modelos con el objetivo de tener un único modelo con mejor capacidad predictiva.
...
La idea de \textit{Bagging} \cite{Breiman1996} es calcular $\hat{f}^1(x),\hat{f}^2(x),...,\hat{f}^B(x)$ usando B muestras \textit{bootstrap}\footnote{El bootstrap es un método de remuestreo que se utiliza para cuantificar la incertidumbre o reducir la varianza de un estimador. Consiste en generar múltiples muestras simuladas a partir de un único conjunto de datos mediante extracciones aleatorias con reemplazo. Al utilizar el reemplazo, cada nueva muestra mantiene el tamaño del conjunto original, pero altera la distribución al repetir ciertas observaciones y excluir otras.} del conjunto de observaciones $(X,Y)=\{(x_1,y_1),(x_2,y_2),...,(x_n,y_n)\}$, y promediar las predicciones para reducir la varianza. Es decir
\begin{equation}
    \hat{f}_{bag}(x)=\frac{1}{B}\sum_{b=1}^{B}\hat{f}^b(x)
\end{equation}

La ventaja de utilizar \textit{Bagging} para predecir es que no hay que preocuparse por la profundidad de los árboles, dado que los árboles individuales tendrán muy poco sesgo y la varianza será reducida al tomar promedios. Además, si bien \textit{Bagging} puede utilizarse en múltiples métodos estadísticos, es particularmente útil para árboles de decisión dado que permite reducir la varianza sin aumentar el sesgo. Como se ve más formalmente a continuación, partiendo de las predicciones

\begin{equation}
    \hat{f}_{bag}(x)=\frac{1}{B}\sum_{b=1}^{B}\hat{f}^b(x)
\end{equation}

y asumiendo que los datos provienen de un modelo de la forma $Y = f(x) + \epsilon$, donde $f(x) = E[Y \mid x]$ es la verdadera función que relaciona las covariables con la respuesta y $\epsilon$ es un término de error con $E[\epsilon] = 0$ y $Var(\epsilon) = \sigma^2$, el error cuadrático medio de la predicción se puede descomponer como

\begin{equation}
    \begin{aligned}
    MSE &= E\Big[\big(Y-\hat{f}_{bag}(x)\big)^2\Big] \\
        &= \Big(E[\hat{f}_{bag}(x)] - f(x)\Big)^2 + Var\Big(\hat{f}_{bag}(x)\Big) + \sigma^2
    \end{aligned}
    \label{eq:bias-variance}
\end{equation}

El primer término, $\big(E[\hat{f}_{bag}(x)] - f(x)\big)^2$, es el sesgo al cuadrado y mide cuánto se desvía, en promedio, la predicción del modelo respecto de la verdadera función $f(x)$. El segundo término, $Var(\hat{f}_{bag}(x))$, es la varianza del modelo y captura cuánto cambian las predicciones al variar la muestra de entrenamiento. El tercer término, $\sigma^2 = Var(Y - f(x))$, es el error irreducible, es decir, la variabilidad intrínseca de los datos que ningún modelo puede capturar. Dado que \textit{Bagging} utiliza árboles de mucha profundidad, el sesgo se mantiene bajo, por lo que la mejora proviene principalmente de la reducción de la varianza. Concentrándose entonces en cómo la varianza se ve afectada

\begin{equation}
    Var\Big(\hat{f}_{bag}(x)\Big)=Var\Bigg(\frac{1}{B}\sum_{b=1}^{B}\hat{f}^b(x)\Bigg)=\frac{1}{B^2}Var\Bigg(\sum_{b=1}^{B}\hat{f}^b(x)\Bigg)
\end{equation}

asumiendo que los $\hat{f}^b(x)$ están idénticamente distribuidos, se puede reescribir:

\begin{equation}
    Var\Bigg(\sum_{b=1}^{B}\hat{f}^b(x)\Bigg)=\sum_{b=1}^B Var\Big(\hat{f}^b(x)\Big) + \sum_{i=1}^B\sum_{j\neq i}^B cov\Big(\hat{f}^i(x), \hat{f}^j(x)\Big)
\end{equation}

ahora asumiendo varianzas iguales ($Var(\hat{f}^b(x))=\sigma^2$) y correlaciones idénticas ($\rho$) para cada par de árboles:

\begin{equation}
    Var\Bigg(\sum_{b=1}^{B}\hat{f}^b(x)\Bigg)=B\sigma^2 + B(B-1)\rho\sigma^{2}
\end{equation}
...
\begin{equation}
    \begin{aligned}
		Var\Big(\hat{f}_{bag}(x)\Big) &= \frac{1}{B^2}\Big(B\sigma^{2}+B(B-1)\rho\sigma^{2}\Big) \\
		&= \sigma^{2}\Bigg(\frac{1}{B}+\frac{\rho B(B-1)}{B^2}\Bigg)
    \end{aligned}
\end{equation}

%#TODO: Decir algo más de como es el boostrap -> Listo! lo sume como nota al pie, pero quizas tendria que ser mas preciso...
...
\textit{Bagging} tiene una forma muy natural de calcular el error, básicamente se basa en el hecho de que cuando se hace una muestra \textit{bootstrap} solamente se toma en cuenta aproximadamente el 63\% de los datos. Para demostrarlo, partimos de la probabilidad de no ser seleccionado en una muestra:

\begin{equation}
P(\text{No ser seleccionado en una muestra}) = 1-\frac{1}{n}    
\end{equation}

Luego, como se realizan $n$ extracciones con reemplazo, por lo que la probabilidad de no ser seleccionado en ninguna muestra es
\begin{equation}
P(\text{No ser seleccionado en ninguna muestra}) = \Bigg(1-\frac{1}{n}\Bigg)^{n}
\end{equation}
	
cuando $n\rightarrow \infty$:
	
\begin{equation}
\lim_{n\rightarrow \infty}\Bigg(1-\frac{1}{n}\Bigg)^{n} = \frac{1}{e}\approx 0.368
\end{equation}
...
Este resultado permite estimar el error usando las observaciones \textit{out-of-bag} como muestra de testeo. Con un $B$ suficientemente grande, este error es asintóticamente equivalente a \textit{leave-one-out cross validation}, lo cual es particularmente útil cuando se cuenta con una muestra grande donde el \textit{leave-one-out cross validation} tradicional sería computacionalmente costoso. Esta relación se evidencia en la \autoref{fig:obb-vs-loocv}, que compara las estimaciones de error \textit{out-of-bag} y \textit{leave-one-out cross validation} utilizando el conjunto de datos \textit{Abalone} y variando la cantidad de árboles $B$. Se observa claramente que, al incrementar el número de árboles, ambas métricas convergen hacia valores similares.

\begin{figure}[ht!]
    \centering
    \includegraphics[width=1\textwidth]{/Users/mcargnel/Documents/mem/book/images/capitulo_3/oob_vs_loocv.pdf}
    \caption{Comparación entre el error de \textit{out-of-bag} y \textit{leave-one-out cross validation} para modelos de \textit{Bagging} con distintos números de árboles ajustados al conjunto de \textit{Abalone}.}
    \label{fig:obb-vs-loocv}
\end{figure}
...
\textit{Random Forest} \cite{breiman2001a} propone una mejora con respecto a \textit{Bagging} al tener en cuenta solo un sub-conjunto de las covariables para cada \textit{split}. Esto permite que los árboles sean distintos entre ellos, haciendo que estén menos correlacionados y por lo tanto reduciendo más el error.
...
\section{Boosting}
%#TODO: el modelo se puede decir algo mas? en regresion es un modelo de regresion? -> No me queda claro que mas agregar
A diferencia de \textit{Bagging}, \textit{Boosting} no promedia predicciones, sino que construye un modelo iterativamente, donde cada modelo se construye con el objetivo de corregir el error cometido por el modelo anterior. A continuación se presenta, para el caso de regresión, un ejemplo de cómo funciona el algoritmo de \textit{Gradient Boosting Machines} (\textit{GBM}) propuesto por \cite{friedman_2001}.
...
donde $x_i$ es un vector de covariables y $y_i$ es la variable de respuesta continua.

El algoritmo de \textit{Gradient Boosting Machine} propone un método para encontrar un aproximación de la función $f(x)$ que minimiza una función de pérdida $L(y,f(x))$:
...
El algoritmo de \textit{Gradient Boosting Machine} requiere un conjunto de datos $(X,Y)$, una función de pérdida $L(y,f(x))$, un número de iteraciones $B$ y un modelo base $h(x,\theta)$. Con esto, el algoritmo es el siguiente:

\begin{enumerate}
    \item Iniciar $\hat{f}_0(x)$ con una constante.
    \item Para cada iteración $b=1,2,...,B$:
    \begin{itemize}
        \item Calcular el negativo del gradiente de la función de pérdida para cada observación $i=1,...,n$:
        \begin{equation}
        -g_{ib} = -\left[\frac{\partial L(y_i, f(x_i))}{\partial f(x_i)}\right]
        \end{equation}
        \item Ajustar el modelo base $h_b(x,\theta_b)$ a los valores $-g_{ib}$.
        \item Encontrar el mejor tamaño de paso $\gamma_b$:
    \begin{equation}
    \gamma_b=\arg\min_{\gamma}\sum_{i=1}^{n}L\Big(y_i,\hat{f}_{b-1}(x_i)+\gamma h_b(x_i,\theta_b)\Big)    
    \end{equation}
    ...
    \end{equation}
    \end{itemize}
    \item Devolver el modelo final $\hat{f}_B(x)$.
\end{enumerate}


\subsection{Funciones de pérdida} 
Dependiendo del problema y los datos con los que se esté trabajando, se pueden utilizar distintas funciones de pérdida. A continuación se presentan algunas de las más comunes para problemas de regresión.
...
Luego, como el gradiente indica la dirección de máximo crecimiento, el negativo del gradiente indica la dirección de máximo decrecimiento. Lo cual es lo que se busca en el algoritmo de \textit{boosting}, es decir, minimizar la función de pérdida. Por lo que el negativo del gradiente evaluado para la observación $i$ en la iteración $b$ resulta ser:

\begin{equation}
- g_{ib} = y_i - \hat{f}_{b-1}(x_i)
\end{equation}
...
Si bien el algoritmo de \textit{Boosting} puede utilizar múltiples modelos base, en este trabajo se expondrán únicamente los modelos base de árboles de decisión. En particular, la literatura sugiere utilizar árboles de poca profundidad en este tipo de modelos. Siendo los \textit{stumps} los árboles de profundidad 1 los más utilizados.

En la \autoref{fig:gbm-depth-effect} se puede ver el efecto de la profundidad de los árboles en el error del modelo donde queda claro como arboles con poca profundidad tienen errores más bajos que los árboles de profundidad mayor.
...
\subsubsection{\textit{Subsampling}}
...
\subsubsection{\textit{Shrinkage}}
\textit{Shrinkage} se utiliza para reducir el efecto de los árboles individuales en el modelo. Intuitivamente, es un hiperparámetro que permite regular cuánto se toman en cuenta los errores de los árboles anteriores para mejorar poco a poco las predicciones.
