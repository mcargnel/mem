...
Se establece la notación matemática que se utilizará de manera consistente a lo largo del documento.

Sea $P_{XY}$ la distribución conjunta inducida por el proceso generador de datos, donde $X$ es una variable aleatoria $p$-dimensional e $Y$ es una variable aleatoria unidimensional. El conjunto de datos se denota como $D_n = \{(x(1), y(1)), \dots, (x(n), y(n))\}$, donde $x(i) \in \mathbb{R}^p$ e $y(i) \in \mathbb{R}$ para $i \in \{1, \dots, n\}$.
