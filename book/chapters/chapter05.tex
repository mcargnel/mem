...
Habiendo identificado las variables más importantes, el próximo paso sería entender cómo estas influyen en la variable de respuesta. Es decir como distintos cambios en la '\textit{frequency}' (Hz) y '\textit{suction-side-displacement-thickness}' (m). Para ello, se puede ver en la \autoref{fig:airfoil-partial-dependence} los ICE (lineas celeste claro)y PDP (linea punteada) de dichas variables, junto con la cantidad de observaciones (líneas verticales en el eje X)
