...
Por último, se analiza la estabilidad de la importancia de variables (PFI) entre los cinco mejores modelos de cada caso. La consistencia en estos resultados sugiere robustez en la interpretación; de lo contrario, podríamos estar ante la presencia de \textit{Rashomon Sets}, donde modelos con capacidades predictivas similares ofrecen explicaciones contradictorias del fenómeno. El objetivo no es necesariamente obtener el modelo perfecto, sino ilustrar la aplicación práctica algunas de las metodologías de interpretabilidad discutidas.

\section{\textit{Airfoil Self-Noise}}

El primer conjunto de datos proviene de la NASA y describe pruebas aerodinámicas y acústicas realizadas en un túnel de viento sobre diferentes secciones de palas \cite{airfoil_self-noise_291}. El objetivo es predecir la presión sonora en decibelios (\textit{scaled-sound-pressure}) a partir de características físicas y operativas. La muestra consta de 1503 observaciones completas (sin valores faltantes) que incluyen perfiles NACA 0012 de distintos tamaños sometidos a diversas velocidades y ángulos de ataque. Las covariables disponibles son la frecuencia (\textit{frequency}), el ángulo de ataque en grados (\textit{attack-angle}), la longitud de cuerda en metros (\textit{chord-length}), la velocidad de flujo libre en metros por segundo (\textit{free-stream-velocity}) y el espesor de desplazamiento del lado de succión en metros (\textit{suction-side-displacement-thickness}).
...
Para comprender qué factores influencian las predicciones del modelo de \textit{Gradient Boosting}, se recurre a la importancia de variables por permutación (PFI). La \autoref{fig:airfoil-permutation-importance} señala a la frecuencia y el espesor de desplazamiento como los predictores dominantes, seguidos por la longitud de cuerda y la velocidad de flujo libre. Es relevante notar que los cinco modelos con mejor desempeño coincidieron en este orden de importancia, otorgando robustez a esta conclusión (\autoref{tbl:imp_airfoil_self_noise}).
...
Para profundizar en la relación entre las variables más importantes y la presión sonora, se utilizan los gráficos de dependencia parcial (\autoref{fig:airfoil-partial-dependence}). En el caso de la frecuencia, se observa una relación inversa clara: los niveles de presión sonora son máximos (130--140 dB) en el rango de bajas frecuencias (0--2500 Hz) y descienden progresivamente hasta los 100--110 dB cerca de los 20000 Hz. El análisis centrado corrobora este efecto, mostrando variaciones de hasta $\pm 20$ dB respecto a la media. No obstante, la escasez de observaciones por encima de los 7500 Hz sugiere interpretar con cautela las predicciones en ese extremo del espectro.
...
\begin{table}[ht!]
    ...
    \hline
    \textit{Cement} & Cantidad de cemento en la mezcla (kg/m³) \\
    \textit{Blast Furnace Slag } & Cantidad de escoria de alto horno (kg/m³) \\
    \textit{Fly Ash} & Cantidad de cenizas volantes (kg/m³) \\
    \textit{Water} & Cantidad de agua en la mezcla (kg/m³) \\
    \textit{Superplasticizer} & Cantidad de superplastificante (kg/m³) \\
    \textit{Coarse Aggregate} & Cantidad de agregado grueso (kg/m³) \\
    \textit{Fine Aggregate} & Cantidad de agregado fino (kg/m³) \\
    \textit{Age} & Edad del concreto en días \\
    \textit{Concrete Comprehensive Strength} & Resistencia a la compresión del concreto (var dep) \\
    \hline
    \end{tabular}
\end{table}

A diferencia del caso anterior, el análisis de correlación (\autoref{fig:concrete_correlation}) no mostró coeficientes superiores a 0.8 en valor absoluto, por lo que se decidió conservar todas las variables para el modelado.
...
Al evaluar el rendimiento predictivo en el conjunto de prueba (\autoref{tbl:concrete_predictions}), \textit{Gradient Boosting} emerge nuevamente como el modelo con mejor desempeño, consolidando la tendencia observada previamente sobre la eficacia de los métodos de ensamble en estos contextos.
...
En resumen, este análisis confirma que la maduración del concreto (edad) y su contenido de cemento son los determinantes críticos de su resistencia, siendo \textit{Gradient Boosting} el algoritmo más capaz de capturar estas relaciones.
...
Antes del modelado, se detectó una alta colinealidad entre el dióxido de azufre total y el libre (\autoref{fig:wine_corr}), procediéndose a eliminar este último.
...
El análisis de dependencia parcial (\autoref{fig:wine_partial_dependence}) muestra el efecto de estas variables. Para el alcohol, la relación con la calidad es positiva y no lineal, con un impacto mayor en el rango medio (11\%--13\%). Por el contrario, la acidez volátil penaliza la calidad de forma casi lineal: a medida que aumenta su concentración, la puntuación predicha disminuye.
...
En conclusión, la aplicación de estas técnicas en tres dominios distintos demuestra su versatilidad. Más allá de la precisión predictiva, donde los ensambles dominaron, herramientas como PFI y PDP permitieron interpretar el efecto de las variables en los modelos más complejos con mayor capacidad predictiva.
