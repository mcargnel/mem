...
pacman::p_load(dplyr, ggplot2, rsample, caret, RColorBrewer, knitr, glmnet, pls, rpart.plot, gbm,tibble, rpart, rpart.plot)
seed <- 1234
doParallel::registerDoParallel()
setwd("~/MEM/Taller datos/TP Final")
vessel_x <- read.table("datos/Vessel_X.txt",sep=",")
vessel_y <- read.table("datos/Vessel_Y.txt",sep=",")
...
Los datos con los que se realiza este análisis son una muestra de `r nrow(df)` vasijas a las que se les realizó un tanto una espectometría de rayos X sobre 1920 frecuencias, como un análisis de laboratorio para determinar el contenido de 13 compuestos químicos: $$Na_2O, \; MgO, \; Al_2O_3, \; SiO_2, \; P_2O_5, \; SO_3, \; Cl, \; K_2O, \; CaO, \; MnO, \; Fe_2O_3, \; BaO \; y \; PbO$$
...
Este trabajo fue realizado en su totalidad con el *software* estadístico R y se organiza de la siguiente manera: en la @sec-analisis-exploratorio se lleva a cabo un análisis exploratorio de los datos con los que vamos a trabajar. Luego se presentan e implementan distintos modelos en la @sec-modelos para después comparar su poder predictivo en términos del error de predicción en la @sec-comp-resultados. Una vez identificado el modelo que tenga el error de predicción más bajo se procede a estudiar en profundidad las predicciones que realiza en la @sec-supuestos. Finalmente, el trabajo concluye en la @sec-conclusiones. En el apéndice se pueden ver en detalle los resultados de cada modelo.
...
En nuestro caso, se implementó PCR usando el paquete *pls* y usando validación cruzada para la estimación del error de predicción. Como se puede ver en la @fig-pca el mejor modelo se construye con `r pcr_fit_componentes` componentes principales como predictoras, el detalle de los 10 resultados con mejor *performance* se puede ver en la @tbl-results-pcr.
...
Así, al hacer la implementación mediante el paquete *pls*, se observa en la @fig-pls (y en detalle en la @tbl-results-pls) que el mejor modelo usando esta metodología (obtenido a través de validación cruzada) se logra con `r pls_fit_componentes` componentes principales como predictoras.
...
A diferencia de *Ridge*, en *LASSO* los coeficientes si pueden truncarse hasta el 0 por lo que el término de *shrinkage* del primer modelo puede considerarse más suave. Para nuestra aplicación podemos ver en la @fig-lasso-cv que el $log(\lambda)$ que minimiza el MSE es `r results_elastic_net %>% filter(Alpha==1) %>% select(log_lambda) %>% first() %>% pull() %>% round(3)`.
...
El modelo de \textit{Elastic Net} es una combinación lineal de las penalizaciones utilizadas por \textit{Ridge} y \textit{LASSO}, por lo que se le suma un hiperparámetro adicional ($\alpha$) que pondera cuanto pesa más una penalización por sobre la otra. Así, para este modelo también se utilizó la técnica de validación cruzada para hallar el mejor modelo, ahora eligiendo la combinación de hiperparámetros $\alpha$ y $\lambda$ que generan un modelo con el menor error de predicción en la muestra de entrenamiento.

En la @fig-elastic-cv se puede ver que el $log(\lambda)$ que minimiza el error de predicción es `r results_elastic_net %>% select(log_lambda) %>% first() %>% pull() %>% round(4)` para un $\alpha$ de `r results_elastic_net %>% select(Alpha) %>% first() %>% pull() %>% round(3)`.
...
#| fig-cap: Estimación del error cuadrático medio por validación cruzada en la muestra de entrenamiento para distintos valores de $\log(\lambda)$ en el modelo Elastic net.
#| echo: false
#| include: true

elastic_df %>% ggplot(aes(x=lambda_log, y=mse)) +
  geom_point(color="#2b8cbe") + theme_minimal() +
  labs(title=expression("MSE en función de " * log(lambda) * " para Elastic-Net"), y = "MSE", x=expression(log(lambda))) +
  theme(plot.title = element_text(hjust = 0.5))
```
...
En la @fig-tree-plot vemos que el árbol con menor MSE es el que tiene 3 nodos terminales\footnote{Este resultado fue muy sencible a distintas semillas, pero las estimaciones del MSE se mantuvieron en rangos similares (por arriba del resto de los modelos), como se estudia en las conclusiones.}, por lo qué podemos referirnos a la @fig-prunned-tree para ver que solamente se usaron V243 y V122 para para predecir. Si bien este modelo es simple e interpretable, no tuvo una gran capacidad predictiva con respecto al resto de los modelos evaluados. Por lo que en la próxima subsección se utilizarán dos métodos de ensambles que se espera mejoren la *performance*.
...
#| fig-cap: Arbol podado a 3 nodos terminales.
#| echo: false
#| include: true
...
Cómo se puede ver en la @fig-rf-plot los resultados obtenidos fueron `r results_rf %>% arrange(mse) %>% select(n_trees) %>% first() %>% pull()` arboles para un sub-conjunto de `r results_rf %>% arrange(mse) %>% select(mtry) %>% first() %>% pull()` covariables. También podemos estudiar en la @fig-importance-rf que las covariables más importantes para predecir el $SO_3$ fueron V246 y V248.

```{r}
#| label: fig-rf-plot
#| fig-cap: MSE del modelo GBM para distintos valores de mtry y ntree.
#| echo: false
#| include: true
...
#| fig-cap: Importancia de las covariables Random Forest ordenadas de mayor a menor.
#| echo: false
#| include: true
...
rf_imp %>%arrange(-IncNodePurity)  %>%head(10) %>% ggplot(aes(x = reorder(variables, IncNodePurity), y = IncNodePurity)) +
  ...
  coord_flip() +
  theme_minimal() +
  labs(title = "Importancias Random Forest", x = "Variable", y = "Importancia") +
  theme(plot.title = element_text(hjust = 0.5))
```
...
Al calcular el MSE para las distintas combinaciones de hiper-parámetros encontramos que el mínimo se alcanza en `r metrics_gbm_df %>% arrange(mse) %>% select(n.trees) %>% first() %>% pull()` para *Boosting* con un *learning rate* de `r metrics_gbm_df %>% arrange(mse) %>% select(shrinkage) %>% first() %>% pull()`. Para entender un poco más qué covariables utilizó el modelo de *Boosting* con menor error de predicción podemos ver la @fig-importance-gbm donde vemos que, V120 fue la covariable más importante en predecir el compuesto $SO_3$.

```{r}
#| label: fig-gbm-plot
#| fig-cap: MSE del modelo GBM para distintos valores de skrinkage y ntree.
#| echo: false
#| include: true
...
metrics_gbm_df %>% 
  ...
  scale_color_brewer(palette = "Dark2") +
  theme_minimal() +
  labs(x = "Número de árboles", y = "MSE", title = "Resultados GBM", color='Shrinkage') +
  theme(plot.title = element_text(hjust = 0.5))
```
...
#| fig-cap: Importancia de las covariables Boosting ordenadas de mayor a menor.
#| echo: false
#| include: true
...
imp_gbm %>% arrange(-Overall)%>%head(10) %>% ggplot(aes(x = reorder(var, Overall), y = Overall)) +
  ...
  coord_flip() +
  theme_minimal() +
  labs(title = "Importancias Boosting", x = "Variable", y = "Importancia") +
  theme(plot.title = element_text(hjust = 0.5))
...
#| tbl-cap: Comparación de modeloes usando las estimaciones del error de predicción con validación cruzada en los datos de entrenamiento.
#| echo: false
#| include: true
...
El modelo que mejor capacidad predictiva tiene para nuestro objetivo de predecir el componente $SO_3$ presente en las vasijas, y sujeto a los datos con los que contamos, es el modelo ajustado *Elastic Net*. El error de predicción (MSE) estimado en la muestra de testeo es `r round(sqrt(mean((test_res$preds_mod_4-test_res$comp_6)^2)), 5)`. Cabe mencionar que esta estimación final del error fue hecha con datos que el modelo no utilizó para su ajuste ni para la comparación entre los otros modelos probados, ya que esas fueron hechas con los datos de entrenamiento y la técnica de validación cruzada para las estimaciones de los errores de cada modelo en dichos datos.
...
#| tbl-cap: MSE para PCA con distintos componentes
#| echo: false
#| include: true
...
#| tbl-cap: MSE para PLS con distintos componentes
#| echo: false
#| include: true
...
#| tbl-cap: MSE para distintos hiperparámetros en Elastic-net, Ridge y LSASO.
#| echo: false
#| include: true
...
#| tbl-cap: MSE para distintos hiperpárametros en Random Forest.
#| echo: false
#| include: true
...
#| tbl-cap: MSE para distintos hiperpárametros en Boosting.
#| echo: false
#| include: true
...
#| tbl-cap: MSE para distintos arboles con distinta cantidad de nodos terminales
#| echo: false
#| include: true
...
#| tbl-cap: Error cuadrático medio para distintos valores de k en K-vecinos
#| echo: false
#| include: true
...
James, G., Witten, D., Hastie, T., & Tibshirani, R. (2013). [An introduction to statistical learning: with applications in R](https://www.statlearning.com/). Corrected edition. New York, Springer.

Hastie, T., Tibshirani, R., & Friedman, J. H. (2009).[The elements of statistical learning: data mining, inference, and prediction](https://hastie.su.domains/Papers/ESLII.pdf). 2nd ed. New York, Springer.
